...
def handle_text_message(event):
    ...
    if text == 'carousel':
        ...
        line_bot_api.reply_message(
            event.reply_token, TextSendMessage(text="hansomemo"))
                ...
                template=ConfirmTemplate(
                    ...
                    actions=[
                        PostbackTemplateAction(
                            label='沒問題',
                            text='沒問題',
                            data='no='+questions[i][0]+'&answer=OK' #questions是整份問卷第幾題 相對題號
                        ),
                        PostbackTemplateAction(
                            label='待改進',
                            text='待改進',
                            data='no='+questions[i][0]+'&answer=NO'
                        )
                    ]
                ))
...
def handle_postback(event):
    userid = event.source['user']#取得Userid


    #Normal丟問題，相對題號
    if event.postback.data == 'Normal':
        if userid in data:#在裡面的話+1
    ...
    #如果Confirm收到12(Normal的最後一題)的話要跳回去Ct
        ...
        #把CT推出去
        line_bot_api.reply_message(
            event.reply_token, CarouselTemplate(columns = ct_container))
